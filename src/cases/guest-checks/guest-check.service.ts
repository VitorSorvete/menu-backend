...
export class GuestCheckService {
  ...
  async create(dto: CreateGuestCheckDto): Promise<GuestCheck> {
    //Regra #1 Não se abre comanda em mesa inexistaente
    ...
    if (!spot) {
      throw new NotFoundException(
        'Não foi encontrado uma mesa ativa com o ID fornecido.',
      );
    }
    ...
    if (opened) {
      throw new ConflictException(
        'Já existe uma comanda aberta para a mesa fornecida.',
      );
    }
  ...
  async close(id: string): Promise<GuestCheck> {
    ...
    //Regra #2: Não posso fechar uma comanda com pedidos que não forem entrgues
    //TO_DO: Implementar isso depois (divida tecnica)
